...
1. `SourceRecordV1` 与 legacy KB document/SQLite row adapter 已完成；外部来源默认 `unverified` 和 quarantine/candidate，不允许治理状态静默降级。
2. 下一步依次建立 `ClaimV1`、`EvidenceV1`、`ResearchPackageV1`，明确 provenance、来源独立性、冲突、未知与风险边界。
