...
|R05|本轮|—|落地调度 worker：`services/python-workers/learning/worker_schedule.py` 复用 donor `LearningScheduler`（非阶梯），stdin JSON→stdout JSON，失败**显式 fail-closed**（返回 error、绝不回退阶梯）；发现 py-fsrs 默认 fuzzing 会让同一 card/rating 产生不同 due → donor 增可选 `enable_fuzzing` 参数（默认 True 保持原行为），worker 用 `False` 保证可复现；测试 8 项（含成熟卡间隔 >14 天阶梯上限、评级敏感、确定性、fail-closed）|IN_PROGRESS（Rust 侧接线未做）|`tests/test_worker_schedule.py`（8 passed）+ donor `tests/test_learning_scheduler.py`（6 passed），14 passed / 0 failed|阻塞/剩余：Rust 尚无调用该 worker 的适配器（需 sidecar request kind 或专用子进程契约），`suggest_next_interval` 仍为 Rust 默认；回滚＝revert 本切片（worker+测试+donor 参数）|
