...
|R03|985a219|—|`cargo test -p archeaxis-archive` exit 0 / 7 组全绿（11 表 v3、13 表 v3 legacy event-key 升级、未知 v3 布局拒绝、缺表拒绝、未知列拒绝）|IMPLEMENTED_PENDING_AUDIT（缺口：尚未用旧实现生成非空 v3 样本）|`crates/archeaxis-archive/src/lib.rs` version_tests、`.project-local/runs/r03-archive.log`|构造 fixture 复原历史形状；需补旧实现导出的非空 v3 包；回滚＝revert 985a219|
