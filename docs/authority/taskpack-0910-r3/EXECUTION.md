...
|R05|（本轮仅 scoping）|—|勘察结论：Rust 仍以 `learning::suggest_next_interval`（1/2/4/7/14 阶梯）为默认排程，正是包内禁止的"临时简单间隔"；FSRS donor `shared/learning_scheduler.py` 暴露 `LearningScheduler.review(card, rating)` 与 `next_due(card)`，6 个 donor 测试健康|IN_PROGRESS（集成面已定位，未接线）|donor 测试 `tests/test_learning_scheduler.py`（6 passed，HEAD 复核）；本行结论与下表|阻塞点：现有 Python worker 契约是文本抽取执行器（请求→text+loss receipt），FSRS 不能直接套用；需二选一——扩展 sidecar 协议新增 scheduler request kind，或单独起调度 sidecar（建议前者，保持 Rust 唯一主库写者）；本轮无代码变更，回滚＝无|
