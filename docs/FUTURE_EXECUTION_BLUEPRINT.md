# ArcheAxis OS Future Execution Blueprint
...
- `ArcheAxis OS Overview`：产品愿景与八个业务域；
...
```text
Learning Goal / Research Question
      → Federated Search → Governed Acquisition → RawAsset
      → Multi-format Conversion → Claim / Evidence → reviewed Knowledge
      → Course / Lesson / Assessment → Human Learning / Mastery
      → governed Machine Knowledge / AI Reuse → Runtime Action
      → Evaluation / Transfer / Conflict → reviewed Knowledge Update
```

其中 `Federated Search → Governed Acquisition` 必须包含受治理的 Web Knowledge Intake：单页、动态网页、整站发现和网页链接中的多格式文件都先形成原始快照与 candidate，不直接成为事实。
...
| Discovery / Web Knowledge Intake | 联邦搜索、单页/整站抓取、动态渲染、多格式路由、网页快照与证据锚点 | Safe HTTP 默认；provider 可替换并隔离；不继承浏览器登录态，不绕过 robots/ToS |
| Knowledge / Archive | 知识版本、关系、冲突、范围与弃用 | 审批和 supersedes 追加，不静默覆盖 |
| Course / Human Learning | 能力图谱、课程、课时、多样式学习对象、练习、掌握与迁移 | 所有教学结论和答案绑定证据；学习行为不等同于事实正确性 |
...
- dynamic profile：从 [`apify/crawlee-python`](https://github.com/apify/crawlee-python)、[`unclecode/crawl4ai`](https://github.com/unclecode/crawl4ai) 和等价候选中，以真实 corpus 选择隔离浏览器 worker；
- site profile：从 Crawlee、[`spider-rs/spider`](https://github.com/spider-rs/spider)、[`scrapy/scrapy`](https://github.com/scrapy/scrapy) 和等价候选中选择受限站点发现、页面流和 checkpoint provider；
...
该 Track 的原始固定任务和威胁模型见 [`taskpacks/MANDATORY_WEB_KNOWLEDGE_INGESTION_ADDENDUM_v1_2026-08-09.md`](taskpacks/MANDATORY_WEB_KNOWLEDGE_INGESTION_ADDENDUM_v1_2026-08-09.md)；品牌绑定按较新的 [`taskpacks/MANDATORY_CAPABILITY_FIRST_KNOWLEDGE_LIFECYCLE_ADDENDUM_v1_2026-08-09.md`](taskpacks/MANDATORY_CAPABILITY_FIRST_KNOWLEDGE_LIFECYCLE_ADDENDUM_v1_2026-08-09.md) 解释。完成 `AXW-WEB-EXIT` 仍是多格式、全闭环和 v1.0 发布资格的补充前置条件。

### Track KLC — Capability-first Knowledge Lifecycle（用户已激活）

产品必须交付 `学习目标/研究问题 → 联邦搜索 → 安全摄取 → 多格式转换 → 证据与知识 → 课程与多样式学习对象 → 人类学习/掌握 → AI 有证据复用 → 评测反馈` 的完整链路。Crawl4AI、Spider、Crawlee、Docling、Open edX、Haystack 等只作为候选能力来源；允许 Adapter、CLI、sidecar、合法 fork/vendor、仅吸收设计或落选，不能以品牌替代能力验收。

后端以现有 RawAsset、DerivedDocument、Evidence、Claim、Knowledge、LearningArtifact、MasterySignal、AI Asset、Job/Outbox 为唯一事实链，补充 SearchPlan、QualityProfile、CourseBlueprint、LessonVersion、Assessment、RetrievalTrace 等必要合同。前端建立 Discovery、Intake、Conversion Quality、Knowledge Review、Course Studio、Learning Player、AI Reuse 和 Evaluation 连贯工作区。

准确率按搜索、网页/文档、OCR/版面、媒体、代码/数据、知识、课程、人类学习和 AI 复用分层评估。原件/来源、证据可解析、伪引用、approved-only 和安全边界使用零容忍门禁；其他阈值在查看结果前冻结，并报告人工真值、盲测、样本量和置信区间。

固定任务、候选组合、自研准入和安装态门禁见 [`taskpacks/MANDATORY_CAPABILITY_FIRST_KNOWLEDGE_LIFECYCLE_ADDENDUM_v1_2026-08-09.md`](taskpacks/MANDATORY_CAPABILITY_FIRST_KNOWLEDGE_LIFECYCLE_ADDENDUM_v1_2026-08-09.md)。`AXW-KLC-EXIT` 是单主题全闭环和 v1.0 资格的补充前置条件。
...
→ W 可替换 Provider 的前后端网页知识摄取
→ I Knowledge / Search / Graph / Memory
→ KLC 搜索—摄取—课程—学习—AI 复用全生命周期资格
...
- 复制或部署完整 Open edX/Moodle/H5P 平台、无边界通用动态 Planner、多 Agent 自治；知识生命周期、课程生产线和人类学习闭环已由 KLC Track 激活，不再属于可选延后项；
