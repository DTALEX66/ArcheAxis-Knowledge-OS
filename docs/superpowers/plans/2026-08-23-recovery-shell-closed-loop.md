...
4. Enumerate backups only from `<runtime data>/data/backups`, require the exact expected filename pattern plus adjacent manifest, canonicalize under the allowed backup directory, and reject absolute paths, separators, parent traversal, or unknown names.
...
6. Preserve existing Job Object shutdown semantics and ensure exit remains non-blocking on Tauri's event loop.
7. Run `cargo test --manifest-path src-tauri/Cargo.toml` from the exact worktree.
...
5. Preserve visible focus and `prefers-reduced-motion`; add responsive recovery layout without changing the six-space information architecture.
6. Run targeted Vitest, then full `npm test` and `npm run build` using the repository-local/external dependency cache already configured by the project.
