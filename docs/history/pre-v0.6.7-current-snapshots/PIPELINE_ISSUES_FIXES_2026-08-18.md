...
| 2 | 图片 OCR 全失败 → **已修复** | 1269 张失败 → 修复后 20/20 成功（课程表/证据矩阵等，~1s/张） | **TESSDATA_PREFIX 指错路径**（toolchains 少了 10- 前缀）→ ocr_adapter.configure_tesseract() 自动解析二进制+语言包 |
| 3 | 6 个 PDF 转化失败 | 无文本输出 | 加密/损坏/纯扫描无文本层且 OCR 兜底缺失 |
| 4 | 音频 ASR 受限 → **链路已验证** | 14 个 mp3 全量转 CPU 成本高（预算跳过），但链路可用：60s 真实中文讲座片段 45s 转录准确 | 非模型缺失，是 CPU 吞吐预算；可 ffmpeg 截段/流式，sherpa-onnx+SenseVoice 为下一步 |
