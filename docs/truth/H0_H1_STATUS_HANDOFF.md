...
## 内容准确率保证机制：高精度识别转译 + 全网交叉对比（2026-08-11 确立）

对**任何格式**（PDF、图片、音频、视频、Office 等）经高精度模型识别转译（OCR/ASR/多格式提取）后的内容，准确率保证采用**两层验证**：

- **识别转译层**（转译得准不准）：高精度模型 + CER/WER 金标准对。
- **内容事实层**（转译出的内容对不对）：**全网交叉比对**（本机制核心，全格式通用）——用公开权威来源（Wikipedia REST API、官方文档、百科、教材、图书馆/学术数据库）对转译出的可核验事实点做交叉校验。
- **适用边界**：所有格式识别转译后都要做全网交叉对比；内部课程用"本地源优先 + 网络校对"（obsidian-web-crosscheck）；识别转译置信度**永不**当作内容事实准确性。
- **流程**：提取可核验事实点 → 选权威来源（主题速查）→ 记录"书 vs 权威精确值"差异（不直接改正文）→ 输出交叉比对表 + 可信度等级（A/B/C/D）+ 待确认项。
- **已验证案例（2026-08-11）**：《时间简史（插图本）》6 点 +《缤纷的语言学》5 点（PDF 格式识别转译后交叉对比），11 点中 9 PASS + 2 待补（哈勃年代精度 1929、霍金无边界来源），无实质错误，可信度等级 A。报告见 `.hermes/task-runtime/web-crosscheck-report-2026-08-11.md`。机制已扩展至全格式（PDF/图片/音频/视频/Office）。
- **工具**：Hermes 技能 `obsidian-web-crosscheck`（v3 已扩展为全格式两层验证：识别转译 + 全网交叉对比；含公开书籍场景权威源速查）。
