...
## 公开内容准确性保证机制（2026-08-11 确立）

对**公开出版书籍/通用参考资料**（非内部课程、非 OCR/ASR 识别管线），内容准确率的保证机制是**全网交叉比对**，而非模型置信度或 CER/WER：

- **定义**：入库/使用前，用公开权威来源（Wikipedia REST API、官方文档、百科、教材、图书馆/学术数据库）对书内可核验事实点做交叉校验。
- **适用边界**：公开书籍/通用参考文本用此机制；内部课程用"本地源优先 + 网络校对"（obsidian-web-crosscheck）；OCR/ASR 识别质量才用 CER/WER 金标准对。三者不混用。
- **流程**：提取可核验事实点 → 选权威来源（主题速查）→ 记录"书 vs 权威精确值"差异（不直接改正文）→ 输出交叉比对表 + 可信度等级（A/B/C/D）+ 待确认项。
- **已验证案例（2026-08-11）**：《时间简史（插图本）》6 点 +《缤纷的语言学》5 点，11 点中 9 PASS + 2 待补（哈勃年代精度 1929、霍金无边界来源），无实质错误，可信度等级 A。报告见 `.hermes/task-runtime/web-crosscheck-report-2026-08-11.md`。
- **工具**：Hermes 技能 `obsidian-web-crosscheck`（v2 已扩展覆盖公开书籍场景 + 书籍权威源速查）。
