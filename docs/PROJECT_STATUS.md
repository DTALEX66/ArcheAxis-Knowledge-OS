...
> **v0.6.4 当前裁决（2026-08-23）**：`v0.6.4 development — PARTIAL`。本轮承接 v0.6.0 最小可信闭环任务包；既有 `v0.6.0` 至 `v0.6.3` 标签按不可改写策略保留且没有 GitHub Release。当前 SHA、能力矩阵与证据层级必须由 `scripts/generate_current_reports.py` 生成的 `reports/current/` 报告和实际门禁运行决定。
...
- 当前源码版本为 `0.6.4`，packaged source manifest 按合同仍为 `unreleased / public=false`；公开稳定 Release `v0.5.0` 与未发布历史标签 `v0.6.0` 至 `v0.6.3` 都不会预先或隐式宣告 v0.6.4 公开发布。
