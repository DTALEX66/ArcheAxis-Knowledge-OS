...
项目正式处于 **Phase 2：版本化 Contracts**。Phase 0 与 Phase 1 已完成；Phase 2 的 `TaskPackV1`、`ExecutionTraceV1`、`EvaluationV1`、`LessonV1` Runtime 合同组及 `SourceRecordV1`、`ClaimV1` 已落地，下一项是 `EvidenceV1`，随后推进 ResearchPackage 与 Knowledge/Learning 合同。部分正式 migration 能力虽已前置完成，但 Phase 3 安全 P0 尚未正式进入；Phase 7–9 的真实 Runtime、统一 Sleep Loop 与 Minimum Complete System Alpha 均未完成。
...
- Phase 2 已建立 `TaskPackV1`、`ExecutionTraceV1`、`EvaluationV1`、`LessonV1`、`SourceRecordV1` 与 `ClaimV1`：TaskPack 支持 KB dataclass/SQLite row 无损往返、Runtime 窄投影，以及可修复历史 `requires_review DEFAULT 0` 的备份式 v3 migration/WAL rollback；Execution Trace 与 Lesson 支持 Runtime/SQLite row 无损往返并对未知行字段 fail closed；Evaluation 支持 Runtime 无损往返；SourceRecord 支持 legacy KB document/SQLite row 无损往返，来源默认 `unverified` 和 candidate；Claim 将 caller-supplied evidence 深拷贝绑定到单一 claim，拒绝 claim ID 冲突、自动 verified 升级和 server-owned claim 向 legacy verifier 降级。
