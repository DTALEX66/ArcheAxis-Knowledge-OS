...
> v0.6.4 修复发布基线（2026-08-23，承接 v0.6.0 任务包）：本页此前的 `b97035e` 快照和“supported”表述均为历史规划输入。前三次修复依次关闭发布安装器重建、schema v3 生产者/消费者/读回器契约漂移，以及 ZIP 生命周期未规范化期望路径的误判；当前状态统一为 `v0.6.4 development — PARTIAL`，只可由 exact-SHA Receipt 与发布资产读回提升；规范实现线为 `frontend/` + 根 `src-tauri/`，详见 `docs/architecture/ADR-060-001-IMPLEMENTATION-LINE.md`。
