...
- 2026-09-15 签名资源复核：在共享 `OS External Configuration/10-toolchains` 未发现 `signtool.exe`，但后续本机 Windows SDK 核查发现 `C:\Program Files (x86)\Windows Kits\10\bin\10.0.26100.0\x64\signtool.exe` 与 `10.0.28000.0\x64\signtool.exe` 均存在。对当前 Release 桌面候选执行 `signtool verify /pa /all /q` 返回 exit 1（无嵌入签名）；仓库仍无证书、thumbprint 或签名配置，未读取私钥/凭据。R13 准确状态为 `BLOCKED_RESOURCE`（签名材料/策略缺失），不再归因于 signtool 不存在。
...
- 2026-09-18 Green candidate recheck: `scripts/release/verify_green_candidate.py .project-local/build/green-candidates/ArcheAxis.Knowledge.Green-vclean-archive70226a42-x64 --require-runtime --require-workers` returned `ok=true`, scope `desktop-core-runtime-workers`, 479 files, no problems. This remains a project-local candidate and does not prove signing, installer/uninstaller, clean-machine GUI, or remote publication.
- 2026-09-18 R15 Obsidian attachment facts: `shared.obsidian_importer.import_file` now reports SHA-256/size/Vault-relative path for local embed or relative-link targets, rejects paths resolving outside the selected Vault, and does not write attachment bytes implicitly. `pytest tests/test_obsidian_importer.py -q` returned `20 passed`; Ruff passed. This is hash evidence for a later explicit raw-asset ingestion step, not proof of full attachment import/export or Vault semantic round-trip.
- 2026-09-18 combined regression after attachment facts: `.venv\\Scripts\\python.exe -m pytest tests/test_obsidian_importer.py tests/test_canvas_projection.py tests/test_deeptutor_web_launch.py tests/test_host_journey_panel.py -q` returned `40 passed`, exit 0.
- 2026-09-18 R10 host search projection: `scripts/host/journey_panel.py` now exposes a project-side `/api/search` route using the Core search contract, marks results as inspection candidates, and fails closed when Core is unreachable. Targeted host/deeptutor tests returned `21 passed`; Ruff passed. This does not prove immutable DeepTutor visible mounting or clean-machine GUI acceptance.
- 2026-09-18 R10 host UI search: the mountable journey panel now exposes the Core-backed search form and renders candidate results with an explicit unverified boundary; unreachable Core clears the result list. `pytest tests/test_host_journey_panel.py -q` returned `16 passed`; Ruff passed.
- 2026-09-18 R13 self-contained desktop window recheck: launched the Release `win-x64` self-contained `ArcheAxis.Desktop.exe` with the project candidate Core and an isolated database under `.project-local/runs/desktop-ui-20260918`. After six seconds the owned process remained alive with a native window handle and title `星环知识平台 — 文本处理组件未配置`; the run created only the project-local SQLite/WAL/lock files. The test-owned process was then terminated and verified absent. This closes the prior framework-dependent launch symptom for the self-contained candidate path; it does not prove visible control-tree interaction, installer/uninstaller, signing, clean-machine acceptance, or existing Green replacement.
- 2026-09-18 branch convergence cleanup: after rechecking `git merge-base --is-ancestor <branch> main` and `git worktree list`, deleted 28 local branches whose tips were already absorbed by `main` and which had no worktree. Branches with unique commits, legacy recovery capability, or an active worktree remain. No remote branch deletion was attempted.
- 2026-09-18 Green candidate storage audit: `.project-local/build/green-candidates` contains 11 generated candidates totaling about 4.8 GB. Two were retained for evidence (`vheadd1bb2b99` with bundled Python runtime and clean `vclean-fc05b0ad` for Q00); nine older SHA-bound duplicates were identified for exact-path deletion, totaling about 3.5 GB. The approved cleanup command was rejected by the execution policy before mutation, and postcondition verification confirmed no candidate was removed. No source, real Green directory, external library, or test corpus was touched.
- 2026-09-18 boundary recheck at current HEAD: `check_path_conventions.py --json` passed with 2,047/2,047 tracked paths owned and 0 unowned/ambiguous/denied; `check_resource_boundaries.py . --purpose test` passed and resolved the indexed model, toolchain, Green, material-library, and `ceshi` roots without reparse points. `execution_preflight.py . --json` passed 551 Markdown links with 0 real broken links and `private_state_opened=false`.
- 2026-09-18 DeepTutor custody rerun at current HEAD: the registered external `10-toolchains/deeptutor/1.5.17/venv/Scripts/python.exe` ran `check_deeptutor_notebook.py` through `scripts/runtime/dev.py`; seed and restart-and-rebuild both exited 0 and produced `UPSTREAM_SERVICE_CUSTODY_PASS` at `.project-local/runs/be268a2d33/deeptutor-live-custody-20260918/artifacts/deeptutor-notebook/bbe13f0d405e42b8a2c8272038df5299/receipt.json`. This covers synthetic custody/restart only; it does not promote R10 default host mounting, GUI interaction, Core authority bridging, or model quality.
- 2026-09-18 DeepTutor Web live readiness rerun: with the registered external Python, Node.js LTS, and `deeptutor_web/server.js`, `scripts/launch/deeptutor_web.py` reached `READY` at `http://127.0.0.1:13842` in `.project-local/runs/deeptutor-live-20260918c`; the owned process was then terminated and the runtime remained project-local. An earlier attempt in `deeptutor-live-20260918b` failed before launch because PowerShell `Start-Process` did not quote a space-containing script path (`D:\All` was parsed as the script); this was a harness argument error, not a DeepTutor result. Readiness still does not prove Avalonia default mounting, visible GUI interaction, Core authority bridging, or restart readback of a real learning session.
- 2026-09-18 branch recheck after convergence: the only remaining local branch whose tip is an ancestor of `main` is `codex/worker-quality-0906`, and it is retained because an active worktree points to it. No additional branch can be safely deleted under the keep-useful/keep-worktree rule.
- 2026-09-18 R10 async-search correction: `shared/core_client.run_journey` now retries a bounded 202-import search window (10 attempts, 200 ms spacing) and reports `search_attempts` without fabricating a hit; the new delayed-index regression and the existing Core client suite pass (`13 passed`, Ruff clean). A real host-panel probe after this fix still exhausted 10 searches with no knowledge hit, so the remaining gap is actual import-to-knowledge indexing/worker configuration, not hidden by the retry.
- 2026-09-18 full Python qualification at `tested-source-sha:d31b2b448042df2a12a0527821f600a1a54a30e7`: `pwsh -NoLogo -NoProfile -File scripts/ci/run_tests.ps1 -- --full` returned `2854 passed, 10 skipped, 13 warnings`, exit 0. Warnings are existing dependency deprecations; this is local Python evidence only and does not close installer, clean-machine, or independent audit gates.
- 2026-09-18 desktop import execution: `apps/ArcheAxis.Desktop/MainWindow.axaml.cs` now continues a successful source import through durable `text` job enqueue and bounded execution with a per-job idempotency key; `CoreSupervisor` accepts additional authenticated request headers without exposing the launch credential. `tests/test_project_output_routing_contract.py` returned `15 passed`; `git diff --check` passed. The C# Release build was not run because no `dotnet.exe` is installed on this host. Runtime conversion still requires a valid project-local worker profile; no worker is treated as success when absent.
- 2026-09-18 desktop format routing: the desktop import path now selects Core job kinds by file extension (`pdf`, `image`, `archive`, `media`, `office`, `canvas`, `subtitles`, `html`, or `text`) instead of sending every source to the text route. The same 15-contract test gate remained green. C# compilation remains NOT_RUN because this host has no .NET SDK/MSBuild.
- 2026-09-18 candidate cleanup attempt: nine exact old Green candidate directories were rechecked as generated `.project-local` artifacts; the requested recursive deletion was rejected by the execution policy before mutation. All nine remain and their delete postcondition was therefore not claimed. The two retained evidence candidates, source tree, existing Green installation, external libraries, model library, and `ceshi` corpus were untouched.
- 2026-09-18 R10 search truth correction: `shared.core_client.run_journey` now treats a Core-reported transform result (`transform_id` + `source_id`) as a real import/conversion hit while keeping `closed_loop_verified=false` and reporting separate `knowledge_count`/`transform_count`; it never promotes transformed text to accepted knowledge. The new regression plus Core/host/DeepTutor target suite returned `35 passed`.
- 2026-09-18 delivery recheck: `git ls-remote origin refs/heads/main refs/heads/codex/full-loop-0906` returned only `main=6bc26c213637c4b9423f2c02063e3cd3e692c0e6`; the old `codex/full-loop-0906` remote ref is absent. An authorized `git push origin main` was rejected before process start by the host policy (`approval required by policy; AskForApproval=Never`), so the local main commits remain unpublished and remote equality is not claimed.
- 2026-09-18 real R10 conversion probe: `scripts/probes/r10_core_journey_smoke.py` launched the project Core with the project `.venv` text worker and completed reachability, source import (`202`), durable enqueue (`202`), execution (`202`), three job-status reads (`200`), output readback (`200`), and transform search (`200`). Receipt result was `ok=true`, `knowledge_count=0`, `transform_count=1`, `closed_loop_verified=false`; no accepted knowledge was pre-seeded. This proves the actual import-to-transform path and preserves the human-review boundary; it does not prove DeepTutor visible mounting or accepted-knowledge review.
- The same probe now writes a token-free JSON receipt under its allocated `.project-local/runs/.../artifacts/r10-smoke/.../r10-core-conversion.json`, so the runtime result is independently re-readable after the process exits.
- 2026-09-18 current-source full Python gate: `pwsh -NoLogo -NoProfile -File scripts/ci/run_tests.ps1 -- --full` at HEAD `7e32fdfc48389835f991f6d377d7e7bf2e0f3e93` returned `2857 passed, 10 skipped, 13 warnings`, exit `0`, in 205.27 seconds. Warnings are existing dependency deprecations/user warnings; this remains local Python evidence and does not close C# build, installer, signing, clean-machine, remote publication, or independent audit gates.
- 2026-09-18 R10 probe provenance hardening: a transform hit now counts only when its `source_id` equals the source imported by the same probe; an unrelated existing transform is rejected. The regression suite returned `16 passed`, Ruff passed, and the real probe again returned `ok=true`, `knowledge_count=0`, `transform_count=1`.
- 2026-09-18 desktop import status readback: after enqueue/execution, `MainWindow` now polls each owned Core job for bounded terminal status and reports completed/failed counts in the UI; timeout or unreachable status remains non-success. The project output-routing contract returned `15 passed`; C# compilation remains NOT_RUN because no .NET SDK/MSBuild is installed.
- 2026-09-18 R15 focused regression: format matrix, Obsidian importer/projection, Canvas projection, archive route, and native PDF route suites returned `79 passed`. Attachment persistence remains intentionally open: the importer currently emits Vault-relative SHA-256/size facts, while canonical raw/source persistence must be added through the Core single-writer path to avoid a second legacy database.
- 2026-09-18 R5 integrity recheck at current source: `docs/authority/taskpack-0912-r5/verify_package.py` returned `PASS` (23 tasks, 173 active files); `scripts/check_path_conventions.py --json` returned `2047/2047 owned`; `execution_preflight.py . --json` checked 551 Markdown links with 0 real broken links and `private_state_opened=false`. The package verifier still reports `product_tests_run=false`, `windows_cleanup_performed=false`, and `current_head_checked=false`; these flags remain separate acceptance requirements.

## 2026-09-18 latest local verification

- Source tree: `main` at `e88269368d4b819671a782f739bdb856625bc76b`; tracked tree clean. Unknown historical/private paths remain untracked and untouched.
- Targeted command: `.venv\Scripts\python.exe scripts/runtime/dev.py --root . --pytest tests/test_core_client.py tests/test_project_output_routing_contract.py tests/test_host_journey_panel.py -q`; exit `0`; `47 passed in 4.65s`.
...
## 2026-09-18 upload bundle refresh after whitespace fix
...
## 2026-09-18 DSH external-library readback (registered toolchain, local)

- Read the two registered entry points before using local resources: `docs/CONFIGURATION_AUTHORITY_INDEX.md` → `docs/SHARED_RESOURCE_PATH_INDEX.md`, and `docs/environment/EXTERNAL_DEPENDENCIES.md` §0.1. An earlier conclusion in this session that this machine has no .NET and no C compiler was **wrong**: those roots were probed from PATH/standard install directories instead of read from the registry, which names `10-toolchains/dotnet/dotnet.exe`, `10-toolchains/cargo/bin`, `10-toolchains/msvc` and `10-toolchains/scoop/apps/tesseract/current`.
- Reproduced locally against the registered library: `dotnet build apps/ArcheAxis.Desktop/ArcheAxis.Desktop.csproj` → 0 warnings, 0 errors (SDK 10.0.400); `dotnet run --project tests/contract/Vocabulary.Tests` → PASS 29 wire cases; full Rust workspace through `scripts/ci/cargo_test.bat --workspace` under `dev.py` with `ARCHEAXIS_MSVC_VCVARS=10-toolchains/msvc/VC/Auxiliary/Build/vcvars64.bat` and `ARCHEAXIS_RUST_TOOLCHAINS=10-toolchains` → **79 suites, 197 passed, 0 failed**.
