...
| B05 | Home, Capture, Library, Reader, Knowledge, Learning, Jobs, Settings and Recovery compositions | named surfaces in `MainWindow.axaml`; Core-backed handlers in `MainWindow.axaml.cs`; persisted Evidence anchor list now reads `/api/v1/evidence/anchors` | `IMPLEMENTED_LOCAL / TESTED_LOCAL_STATIC / TESTED_LOCAL_PORTABLE` | Evidence bundles, Editor, Graph and citation picker remain Core-bound |
