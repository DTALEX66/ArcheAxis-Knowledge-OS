...
| --- | --- | --- | --- | --- |
| OP-001 | Exact SHA `9217c510` CI `33667525835` failed fail-closed. Lint found an unannotated legacy-name occurrence and four generated schema files without final LF. Wheel smoke's fake `tesseract` emitted no version string; the OCR resolver correctly rejected it, returning an error object rather than `text`. | Inspect only the seven staged paths with `git diff --cached --check` and run the scoped CI-contract test using the project shared interpreter. | Commit one reviewed repair batch, then bind a new exact-SHA Actions result to lint, wheel smoke and `a0-gates`; only afterward run full qualification. | **OPEN**. [Current Reality](CURRENT_REALITY_2026-09-01.md), [G0 gaps](AXM_G0_EVIDENCE_GAP_REGISTER_2026-09-03.md). |
