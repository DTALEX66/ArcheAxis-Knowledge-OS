...
| --- | --- | --- | --- | --- | --- |
| G0-001 exact qualification | Exact source SHA, lock hashes and all required CI jobs for that SHA | Historical `db13d056` selected CI and nightly remain evidence only. The later `9217c510` CI run `33667525835` executed broad jobs but failed fail-closed at lint (legacy-name placement plus four generated schema final-newline violations) and wheel smoke (fake `tesseract` lacked a resolver-recognised version string). Seven scoped repairs are staged locally, not committed or cloud-verified. | Release/CI operator with an authorised GitHub session | GitHub Actions readback bound to the repair SHA and every required job | **OPEN**. No G1 writer or platform-support claim. |
| G0-002 current truth readback | Current-state record reconciled against CI, release and installed runtime receipts | Current Reality now names the newer `9217c510` failure and staged-repair boundary; documentation link/authority regression passes. Exact CI repair readback and visible installed runtime evidence remain absent. | Project maintainer | Run document authority contracts and compare each cited live claim to its named receipt | **PARTIAL**. Do not infer installed runtime or full CI from a document. |
