...
## 五、ArcheAxis 0906 现场修复规则

- 构建路径改动必须追完整调用链：实际 `--config`、资源输入、Cargo 输出、缓存、制品收集必须一致；不能只改同名兼容目录。
- 收据必须绑定当前运行与源码 SHA，检查全部步骤；仓库中已有的历史 `PASS` 文件不是本次运行证据。
- 备份先在 staging 校验数据库和全部原件，成功后再无覆盖发布；恢复的校验和复制必须保持同一读快照。
- 子进程启动不能以“固定端口有人响应”代替工作区归属；使用自有进程、实际分配端口、静默启动、限时读取和双流消费。
- 质量计算的文本与 SHA 必须来自同一次原始字节读取；不允许隐式去空白/BOM/换行后仍声称无归一化。覆盖率分母必须是全部输入，不是被截断后的数量。
- 路径迁移保留历史证据，更新当前索引；开发输出统一 `.project-local`，不因此删除 `.hermes` 中未经分类的资料。
- R5 执行台账及 `R5-STATE.json` 是历史收据，不再表示当前进度。当前执行基线是 [R6 执行台账](docs/current/R6-EXECUTION.md)、[R6 状态](docs/current/R6-STATE.json) 和 [M0 方向覆盖](docs/current/M0-DIRECTION-OVERRIDE-20260920.md)；定向通过不等于全链路交付。


## 六、2026-09-26 Git/SSH 认证诊断收口

- 本次失败操作为 Git 远端只读查询，实际传输为 SSH；现有 Git URL 重写会把 GitHub HTTPS 地址转换为 SSH，不能根据文档中的静态 URL 判断协议。
- 沙箱执行身份下出现主机记录访问拒绝及路径解析差异；经正常审批在实际用户环境执行，现有认证可用，`ssh -T` 认证成功，`git ls-remote --heads origin` 成功读取 7 个分支。SSH agent 不可达并未阻止该认证。此前“必须恢复认证/启动 agent/生成密钥”的推断不成立。
- 本次只读验证未证明 push、提交签名或分支写入权限。普通编辑与测试仍在原沙箱；已授权且确需远端认证的操作走本机已验证的正常审批路径，不反复重试相同失败环境。审批不可用或拒绝时报告具体命令和原因，不改写成用户没有认证。
- 不读取、输出或复制私钥、令牌、密码和认证文件；不修改 ACL、主机记录、全局 Git 配置或主机校验策略。读取成功不是写入授权。
- 认证诊断已收口，不再列为本项目模型索引与本地管线的未解决阻塞。本记录仅保留脱敏结论，不记录本机账户、私钥位置或认证内容。
