...
> **v0.6.2 修复发布基线（2026-08-23）**：本轮承接 v0.6.0 最小可信闭环任务包；`v0.6.0` 与 `v0.6.1` 标签均已被历史流程占用且无 GitHub Release。`v0.6.1` 暴露了发布身份注入后未重建 NSIS 的门禁缺陷，依据标签不可改写策略，公开候选递增为 v0.6.2。当前结论仍为 **`v0.6.2 development — PARTIAL`**，只能由当前 exact-SHA 的 Golden Journey、Windows 生命周期、CI 与发布资产读回证据提升。
...
- **当前版本**：`0.6.2`；源码 Release Manifest 保持 `unreleased / public=false`。`v0.6.0` 与 `v0.6.1` 是保留且无 GitHub Release 的历史标签，公开 `v0.5.0` 是历史稳定 Release；它们都不能替代 v0.6.2 的 exact-SHA、CI、安装或资产读回证据。
