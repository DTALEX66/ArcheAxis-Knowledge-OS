...
> **v0.6.7 修复发布基线（2026-08-23）**：本轮承接 v0.6.0 最小可信闭环任务包；`v0.6.0` 至 `v0.6.6` 标签均已被历史流程占用且未公开发布。连续修复已关闭发布安装器重建、schema v3 契约、ZIP 路径规范化、ZIP 关闭窗口竞争、Portable 数据库读回路径误判，以及 PowerShell 依赖锁属性路径插值错误；依据标签不可改写策略，公开候选递增为 v0.6.7。当前结论仍为 **`v0.6.7 development — PARTIAL`**，只能由当前 exact-SHA 的 Golden Journey、Windows 生命周期、CI 与发布资产读回证据提升。
...
- **当前版本**：`0.6.7`；源码 Release Manifest 保持 `unreleased / public=false`。`v0.6.0` 至 `v0.6.6` 是保留且未公开的历史标签，公开 `v0.5.0` 是历史稳定 Release；它们都不能替代 v0.6.7 的 exact-SHA、CI、安装或资产读回证据。
