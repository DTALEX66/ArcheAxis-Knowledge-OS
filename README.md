...
> **v0.6.4 修复发布基线（2026-08-23）**：本轮承接 v0.6.0 最小可信闭环任务包；`v0.6.0` 至 `v0.6.3` 标签均已被历史流程占用且无 GitHub Release。前三次修复依次暴露发布安装器重建、schema v3 契约与 ZIP 路径规范化问题；依据标签不可改写策略，公开候选递增为 v0.6.4。当前结论仍为 **`v0.6.4 development — PARTIAL`**，只能由当前 exact-SHA 的 Golden Journey、Windows 生命周期、CI 与发布资产读回证据提升。
...
- **当前版本**：`0.6.4`；源码 Release Manifest 保持 `unreleased / public=false`。`v0.6.0` 至 `v0.6.3` 是保留且无 GitHub Release 的历史标签，公开 `v0.5.0` 是历史稳定 Release；它们都不能替代 v0.6.4 的 exact-SHA、CI、安装或资产读回证据。
