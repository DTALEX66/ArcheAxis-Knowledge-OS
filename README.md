...
- **当前版本**：`0.6.13`（管线与多格式控制台已并入 canonical shell；`v0.6.11` 仍是当前公开稳定版，UI 纠偏随后续版本发布）；源码 Release Manifest 按合同保持 `unreleased / public=false`，公开 artifact identity 为 `stable / public=true`；历史标签均不可改写。
