...
def enqueue_command(
    *,
    ...
    request_fingerprint = sha256(
        _canonical_json(
            {"aggregate_id": aggregate_id, "command_type": command_type, "payload": payload}
        ).encode("utf-8")
    ).hexdigest()
    ...
    with closing(sqlite3.connect(database)) as connection:
        ...
        try:
            ...
            connection.execute(
                ...
                "job_id, command_id, job_type, aggregate_id, state, payload_json, correlation_id, causation_id, created_at, updated_at"
                ") VALUES (?, ?, ?, ?, 'queued', ?, ?, ?, ?, ?)",
                (job_id, command_id, command_type, aggregate_id, request_json, command_id, command_id, timestamp, timestamp),
            )
            connection.execute(
                "INSERT INTO workspace_outbox_v1("
                "event_id, job_id, event_type, payload_json, state, created_at, updated_at"
                ") VALUES (?, ?, ?, ?, 'pending', ?, ?)",
                (event_id, job_id, command_type + ".queued", request_json, timestamp, timestamp),
            )
            connection.execute(
