"""Builtin converter plugin registration: DOCX (AXW-CAP-503).

Registers the existing ``app.ingestion.docx_adapter`` as a builtin
...
``app.ingestion.docx_adapter.convert_docx`` function into a store-callable
``FileConverter`` (convert(source_path, options) -> AdapterResult) and
registers it with ``app.capability.conversion`` so
...
ENTRY_POINT = f"{ADAPTER_MODULE}:convert_docx"
...
MANIFEST: dict[str, Any] = {
    ...
    "data_ownership": {
        "declared": True,
        "note": "converts user-provided .docx files; extracted text blocks are stored in the workspace vault",
    },
...
def get_activator() -> Callable[[], FileConverter]:
    ...
    REAL ``app.ingestion.docx_adapter.convert_docx`` function and
