"""Builtin converter plugin registration: HTML (AXW-CAP-503).

Registers the existing ``app.ingestion.html_adapter`` as a builtin
...
``app.ingestion.html_adapter.convert_html_file`` function into a
...
ADAPTER_MODULE = "app.ingestion.html_adapter"
ENTRY_POINT = f"{ADAPTER_MODULE}:convert_html_file"
ADAPTER_FUNCTION = "convert_html_file"

MANIFEST: dict[str, Any] = {
    "manifest_version": "1.0",
    "plugin_id": "ax.builtin.converter.html",
    "name": "HTML Converter (builtin)",
    "version": "1.0.0",
    "api_contract": "1.x",
    ...
    "data_ownership": {
        "declared": True,
        "note": "converts user-provided .html files; main-content blocks are stored in the workspace vault",
    },
...
def get_activator() -> Callable[[], FileConverter]:
    ...
    REAL ``app.ingestion.html_adapter.convert_html_file`` function and
