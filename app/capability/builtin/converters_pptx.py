"""Builtin converter plugin registration: PPTX (AXW-CAP-503).

Registers the existing ``app.ingestion.pptx_adapter`` as a builtin
...
``app.ingestion.pptx_adapter.convert_pptx`` function into a
...
ADAPTER_MODULE = "app.ingestion.pptx_adapter"
ENTRY_POINT = f"{ADAPTER_MODULE}:convert_pptx"
ADAPTER_FUNCTION = "convert_pptx"

MANIFEST: dict[str, Any] = {
    "manifest_version": "1.0",
    "plugin_id": "ax.builtin.converter.pptx",
    "name": "PPTX Converter (builtin)",
    "version": "1.0.0",
    "api_contract": "1.x",
    ...
    "data_ownership": {
        "declared": True,
        "note": "converts user-provided .pptx files; slide-anchored text blocks are stored in the workspace vault",
    },
...
def get_activator() -> Callable[[], FileConverter]:
    ...
    REAL ``app.ingestion.pptx_adapter.convert_pptx`` function and
