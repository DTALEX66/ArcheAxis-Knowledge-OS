...
def create_machine_knowledge_candidate_on_connection(
    ...
    row = connection.execute(
        "SELECT signal_json, calculated_at FROM mastery_signals_v1 WHERE id=?", (signal_id,)
    ).fetchone()
    ...
    with sqlite3.connect(database) as connection:
        ...
        try:
            ...
            if existing is not None:
                ...
                connection.commit()
                return current
            if current.lifecycle_status == "deprecated":
                raise RuntimeError("deprecated machine knowledge lifecycle is terminal")
