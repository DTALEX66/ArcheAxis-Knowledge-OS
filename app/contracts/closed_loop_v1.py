    ...
    def validate_loop(self) -> "ClosedLoopReceiptV1":
        ...
        if self.overall_status == "complete":
            ...
            for stage in self.stages:
                if not stage.evidence_refs or any(not ref.strip() for ref in stage.evidence_refs):
                    raise ValueError(f"complete closed loop requires {stage.stage} evidence refs")
