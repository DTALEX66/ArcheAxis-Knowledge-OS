...
_FACT_PATTERNS: list[tuple[str, str]] = [
    ...
    (
        r"(\b[A-Z][a-zA-Z]*(?:\s+[a-zA-Z]+){0,3})\s+(is|was|are|were)\s+(a\s+)?(.+?)(?:\.|,|\sand\s|$)",
        "is_a",
    ),
    (
    ...
    (
        r"(.+?)\s+(uses|using|utilizes|employs|leverages|supports|requires)\s+(.+?)(?:\.|,|\sfor\s|$)",
        "uses",
    ),
    (
        r"(.+?)\s+(consists of|contains|includes|comprises|is composed of)\s+(.+?)(?:\.|,|\sand\s|$)",
        "contains",
    ),
    (
        r"(.+?)\s+(causes|leads to|results in|produces|generates|triggers)\s+(.+?)(?:\.|,|\sby\s|$)",
        "causes",
    ),
    (
        r"(.+?)\s+(depends on|relies on|is based on|is derived from)\s+(.+?)(?:\.|,|\sand\s|$)",
        "depends_on",
    ),
    (
        r"(.+?)\s+(is part of|belongs to|is a member of|is a type of)\s+(.+?)(?:\.|,|\sand\s|$)",
        "part_of",
    ),
    (
        r"(.+?)\s+(is similar to|resembles|is like|is analogous to)\s+(.+?)(?:\.|,|\sbut\s|$)",
        "similar_to",
    ),
]
...
_OF_PATTERN = re.compile(
    r"(\b[A-Z][a-zA-Z]+(?:\s+[A-Z][a-zA-Z]+){0,2})\s+of\s+(\b[A-Z][a-zA-Z]+(?:\s+[A-Z][a-zA-Z]+){0,2})"
)
