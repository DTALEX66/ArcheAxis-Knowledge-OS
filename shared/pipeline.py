...
def run_pipeline(
    ...
    if source == "url":
        ...
        title = ext.get("title", "")
        result["stages"]["extract"] = {"engine": ext.get("engine"), "chars": ext.get("char_count", 0)}
    elif source == "text":
        content = input_data
        ...
        result["stages"]["extract"] = {"engine": "duckduckgo+trafilatura", "results": len(sr.get("extracted", []))}
    ...
    if "tag" in actions:
        from shared.auto_tagger import suggest_tags, extract_keywords, detect_atomicity
        tags = suggest_tags(content, max_tags=8)
        keywords = extract_keywords(content, top_k=10)
        atomic = detect_atomicity(content)
        result["stages"]["tag"] = {"tags": tags, "keywords": [k["keyword"] for k in keywords[:5]],
    ...
    if "index" in actions and auto_ingest:
        ...
        doc = {"id": doc_id, "title": title[:200], "content": content[:10000],
               "source": f"pipeline:{source}", "tags": tags}
