...
from shared.storage import select_all, insert  # noqa: E402
...
def weekly_summary(days: int = 7) -> dict[str, Any]:
    ...
    # Gather data
    reviews = [r for r in select_all("kb_reviews", limit=500)
               if (r.get("created_at", "") or "")[:10] >= cutoff[:10]]
    cards = [c for c in select_all("kb_cards", limit=500)
             if (c.get("created_at", "") or "")[:10] >= cutoff[:10]]
    mistakes = [m for m in select_all("kb_mistakes", limit=500)
                if (m.get("created_at", "") or "")[:10] >= cutoff[:10]]
    mkus = [m for m in select_all("machine_knowledge_units", limit=500)
            if (m.get("created_at", "") or "")[:10] >= cutoff[:10]]
    daily_notes = [d for d in select_all("daily_notes", limit=50)
                   if d.get("date", "") >= cutoff[:10]]

    # Compute stats
    review_count = len(reviews)
    avg_quality = round(
        sum(r.get("quality", 0) for r in reviews) / max(review_count, 1), 1
    )
    new_cards = len(cards)
    ...
    if new_cards == 0:
        recommendations.append("No new cards this period. Consider importing new knowledge sources.")
    if mistakes_found > 0 and mistakes_resolved == 0:
        recommendations.append(f"{mistakes_found} mistakes recorded but none resolved. Review them.")
...
def generate_daily_missions() -> dict[str, Any]:
    ...
    mastered = [c for c in select_all("kb_cards", limit=200)
                if c.get("review_status") == "mastered"]
    if mastered:
        missions.append({
            "type": "publish",
            "priority": "medium",
