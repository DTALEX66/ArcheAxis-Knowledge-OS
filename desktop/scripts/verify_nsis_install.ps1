...
try {
    ...
    if ($workspaceStatus -ne 200 -or $status.release.version -ne '0.6.9') {
    ...
    if ($upgradedStatus.release.version -ne '0.6.9') {
    ...
    if ($reinstalledStatus.release.version -ne '0.6.9' -or
        -not (Test-Path -LiteralPath $persistenceSentinel -PathType Leaf)) {
