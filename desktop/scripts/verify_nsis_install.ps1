...
try {
    ...
    if ($workspaceStatus -ne 200 -or $status.release.version -ne '0.6.2') {
    ...
    if ($RequireReleaseIdentity) {
        ...
        if (
            ...
            $version.release.tag -ne 'v0.6.2' -or
    ...
    if ($upgradedStatus.release.version -ne '0.6.2') {
    ...
    if ($reinstalledStatus.release.version -ne '0.6.2' -or
        -not (Test-Path -LiteralPath $persistenceSentinel -PathType Leaf)) {
