...
try {
    ...
    if ($workspaceStatus -ne 200 -or $status.release.version -ne '0.6.8') {
    ...
    if ($RequireReleaseIdentity) {
        ...
        if (
            ...
            $version.release.tag -ne 'v0.6.8' -or
    ...
    if ($upgradedStatus.release.version -ne '0.6.8') {
    ...
    if ($reinstalledStatus.release.version -ne '0.6.8' -or
        -not (Test-Path -LiteralPath $persistenceSentinel -PathType Leaf)) {
