    ...
    <article className="archive-ledger">
      ...
      {entries.length === 0 ? <p className="muted">暂无可验证数据</p> : (
        <dl>{entries.slice(0, 6).map(([key, value]) => (
          <div key={key}><dt>{label(key)}</dt><dd>{valueOf(value)}</dd></div>
        ))}</dl>
      )}
    </article>
  );
    ...
    <section className="space-view workspace-overview" aria-labelledby="workspace-title">
      ...
      <section className="workbench-hero">
        ...
        <aside className="next-actions" aria-label="下一步">
          ...
          <article><span>01</span><div><strong>查看原件与锚点</strong><small>阅读、搜索和安全回写。</small></div></article>
          <article><span>02</span><div><strong>检查证据生命周期</strong><small>支持、反驳、背景与适用范围。</small></div></article>
          <article><span>03</span><div><strong>处理任务与回执</strong><small>失败保留原件，可恢复重试。</small></div></article>
        </aside>
      </section>

      {loading ? <Loading label="工作台" /> : error ? <DataError label="工作台" message={error} /> : (
        <section className="archive-columns" aria-label="工作区真实状态">
          <Ledger title="发布与计数" rows={{ ...(status?.release ?? {}), ...(status?.counts ?? {}) }} />
          ...
          <article className="archive-ledger">
            ...
            {Object.keys(status?.capabilities ?? {}).length === 0 ? <p className="muted">暂无能力状态</p> : (
              <ul>{Object.entries(status?.capabilities ?? {}).map(([capability, state]) => (
                <li key={capability}><span>{label(capability)}</span><strong>{valueOf(state)}</strong></li>
              ))}</ul>
            )}
          </article>
          <article className="archive-ledger">
