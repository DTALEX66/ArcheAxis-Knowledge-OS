...
export function ActivityDock({ onInspect }: { onInspect?: (target: InspectionTarget) => void }) {
  ...
  async function operate(action: () => Promise<{ status: string }>) {
    try {
      ...
      setDelivery(`投递状态：${result.status}`);
      await refresh();
      setDelivery(`投递状态：${result.status}`);
    } catch (error) {
  ...
  return (
    <footer className="activity-dock" aria-label="活动坞">
      ...
      {items.slice(0, 3).map((item) => <span className="activity-dock-item" key={item.public_ref}>{item.label} · {item.state} <button type="button" onClick={() => void inspect(item)}>查看活动详情</button></span>)}
      ...
      <button type="button" disabled title="当前 Outbox 协议尚未定义可撤销状态">取消投递（不可用）</button>
    </footer>
  );
}
